...
public class Server implements HttpHandler {
    ...
    private void doPost(HttpExchange httpExchange) {
        try {
            MessageExchange.Post post = messageExchange.getClientPost(httpExchange.getRequestBody());
            System.out.println("Get Message from " + post.user + " with id " + post.id + " : "+ post.message);
            history.add("id: " + post.id + ", user: " + post.user + ", message: " + post.message);
    ...
    private void doPut(HttpExchange httpExchange) {
        try {
            MessageExchange.Post post = messageExchange.getClientPost(httpExchange.getRequestBody());
            for (String i : history) {
                if (i.regionMatches(4, post.id, 0, post.id.length())) {
                    int index = history.indexOf(i);
                    String sub = "";
                    for (int j = 12; j < i.length(); j++) {
                        if (i.charAt(j - 2) == ':' && i.charAt(j - 9) == 'm') {
                            sub = i.substring(j, i.length());
                        }
                    }
                    i = i.replaceAll(sub, post.message);
                    history.set(index, i);
                }
            }
        } catch (ParseException e) {
